...
- **Black inline-code preference**: use Notion native inline `code` for project paths, repo/local addresses, filenames, commands, config keys, and other literal technical tokens that need monospace emphasis—especially any path or project address. Keep the inline-code font color black/default (`annotations.code=true`, `annotations.color="default"`); do not color the code text gray/brown/red/blue/etc. Do not drop the `code` annotation and fall back to plain body text merely to avoid Notion’s default code chrome. Ordinary prose terms and model names may stay as normal black text when they are not literal paths/addresses/tokens. Write mathematical notation and short expressions as native inline equations; use display equations only for multi-line derivations or separately referenced formulas.
