...
- Before uploading or inserting a paper-card figure, visually inspect the selected file or contact sheet and reject all-black / near-all-black / mostly dark low-information artifacts: web hero backgrounds, dark-mode browser screenshots, transparent PNGs rendered on black, loading/failure frames, UI wrappers, or screenshots whose actual figure content is unreadable. Allow dark backgrounds only when the official paper figure itself is intentionally dark and the content, labels, legends, and caption alignment remain readable. For a two-image teaser + method group, both images must pass this check independently; omit a weak black teaser rather than keeping it. If a figure appears black after Feishu upload or fetch-back, inspect the source pixels/dimensions/thumbnail and replace the block with a verified official asset, MinerU extraction, or PDF crop.
