...
- **User default scope: “迁移” / “migrate” includes the specified source page and all descendant pages, recursively, even when the user supplies only the root URL.** Do not ask whether to include children. Narrow this scope only when the user explicitly says “仅此页” / “不含子页面” or specifies another boundary. This applies to migration, not ordinary page edits or repairs.
- Before writing, enumerate the actual source hierarchy through the platform's child-node/page listing, following pagination at every depth. A body-only fetch or absence of child links in the body does not prove that a page has no children. Traverse descendants, not arbitrary linked pages outside the subtree; track source node IDs to avoid duplicate traversal or shortcut cycles.
- Keep a source-to-destination page map (source ID/URL, source parent, destination ID/URL, verification status). Recreate the source parent-child structure: when asked for a new child page under a destination, place the source root there and nest its descendants beneath that new page. On retries, reuse verified mapped pages instead of creating duplicates; do not treat title alone as identity.
- Verify every migrated page against its source: body, images, captions, tables, formulas, links, and paper-card formatting where present. Preserve each page's source URL at its opening and use the page map for links between migrated pages. Reconcile the enumerated source tree with the destination tree, including parent relationships, not just page counts. Report migrated/verified counts and any excluded, inaccessible, unsupported, or failed nodes; incomplete enumeration or any unverified descendant prevents claiming the whole migration is complete.
