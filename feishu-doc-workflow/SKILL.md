...
When organizing dated research-group meeting pages, use the user's dated meeting page as the durable destination. Route Feishu vs Notion vs Obsidian with [`research-doc-workflow`](../research-doc-workflow/SKILL.md); do not keep a full second copy in the local vault unless the user explicitly asks. Keep the finished page structure and section order stable across meetings; only the input layout and evidence sources may change.
...
- **Attachments or child pages** = optional backups or legacy sources before organization. DOCX files or separately organized exports may exist, but do not rely on an attachment alone when the user can provide readable text in chat.

When organization starts, create **exactly two** children under the dated meeting page if they do not already exist:

1. `notes` — move or copy the personal notes from the top of the main page here, preserving native images, links, wording, and original image dimensions.
2. `原始材料：千问纪要与原文` — archive both Qwen/Doubao raw texts **inside this one page**. Use two headings on that page, in this order: `## 纪要` then `## 原文`. If the transcript is too long for inline text, keep the `## 原文` heading and attach the full transcript file under it. Legacy Doubao pages may still be titled `原始材料：豆包转写`; the interior headings stay the same.

Treat chat-pasted `原文` / `纪要` as source material for the report. Do not embed those raw texts into the report body unless the user explicitly asks.

**Do not** create sibling children named `纪要` or `原文`. **Do not** create a third index page that only links to those siblings. If a long transcript must live as its own page, nest it **under** `原始材料：千问纪要与原文`, never as a sibling of `notes`.
...
- Write the organized report on the dated main page. Keep the report body focused on the organized analysis. Do not delete or replace the original images, links, attachments, or existing raw material.
- Use this default report structure (do not reorder or rename these sections): `组会整理报告`, `Paper Cards`, `保守纪要`, `讨论脉络 / 老师反馈`, `会后头脑风暴`, `TODO`. Stop the report at `TODO`.
- Do **not** add a footer section named `原始材料`, and do not mention `notes` or `原始材料：千问纪要与原文` again in the report body. Those two pages are already the dated page's children and the host shows them at the bottom; a second list repeats them. On Notion, `replace_content` must still preserve only those two children with `<page>` tags so they are not deleted; extra sibling `<page>` blocks for `纪要` / `原文` recreate the bloated tree.
- Do **not** add a reader-facing ASR / 纪要校正 callout, glossary, or `转写疑似` note. Apply confirmed corrections in the prose itself (for example write 单目 when the speaker means 单目). Leave the uncorrected transcript only in the archival raw-material page.
...
- End with concrete TODOs from the meeting plus any follow-up paper-card verification tasks. Fetch after writing and verify that the report sections exist, the dated page has exactly the two intended children (`notes` and the archival raw-material page), the archival page contains both `纪要` and `原文`, image dimensions/captions are preserved, and the child tree did not grow extra `纪要` / `原文` siblings.
...
- Do not write mechanism interrogation, evidence-boundary analysis, counterfactual predictions, or alternative explanations into the default page. Publish them only in a `中文精读稿` the user explicitly requested. Keep the complete Chinese translation source-faithful.
...
- Feishu single-paper deep dives follow the same manuscript-only contract as `paper-deep-dive`. The required durable artifact is the complete Chinese manuscript. Do not create `英文原文稿` or `中文精读稿` unless the user explicitly requests that artifact in the current task.
- `中文精读稿` is not a default child. Create it only on an explicit request.
- An internal English scaffold is the working source for translation. Publish `英文原文稿` only when the user explicitly requests it. That page is the paper's original English text in source order, checked against official HTML / LaTeX / PDF, and it must preserve figure/table positions, formulas, captions, appendices, body citation markers, and a numbered References section.
...
- When explicitly requested, `中文精读稿` is an analytical Chinese reading guide. It follows the source paper's original section order: Abstract, numbered main-paper sections, named subsections, conclusion, then appendices / supplementary material. Do not reorganize it into a purely thematic guide unless the user explicitly asks for a theme-based reader.
- If the complete Chinese manuscript cannot be produced because the source PDF is inaccessible, parsing fails, the official HTML is unavailable or incomplete, or licensing/copyright constraints apply, stop and tell the user what is blocking completion. Do not silently downgrade to a simplified reader or mislabeled excerpt page. Page length is not a reason to omit the manuscript.
- The default Feishu page is the complete Chinese manuscript (`原文译稿` / `完整中文稿`), with formulas, figure/table placements, citations, captions, and the paper's original image order. Insert paper figures as native Feishu image blocks near their original figure captions whenever reliable official images are available. Do not turn that page into paper-card notes, and do not add child pages for `英文原文稿` or `中文精读稿` unless the user explicitly requests them.
...
- Verify the Feishu page is the complete Chinese manuscript and has no `中文精读稿` or `英文原文稿` child unless the user explicitly requested that child.
...
- Use the canonical `paper-card-delivery` format: exact official English title, compact metadata paragraph, direct PDF link, `Dataset:`, native method/process image with complete Chinese caption, and exactly seven Chinese bullet slots: `定义`, `问题`, `方法`, `实现`, `结论`, `局限`, `启发`.
- Paper-card work is source verification, not formatting. Do not mechanically convert old summaries into the canonical format or fill missing slots from plausibility. If official full-paper verification is incomplete, mark the affected metadata or bullet as `待核验`, `未报告`, `Not reported`, `N/A`, or keep the card as an incomplete candidate.
- For Feishu delivery, preserve the compact metadata as one normal text block with four non-empty hard-break lines after the `####` title: takeaway, `Venue｜Institution`, `PDF｜Project｜Code`, and `Dataset:`.
- Insert the native image block after that metadata block and before the seven fixed bullet slots. Preserve existing image blocks, captions, dimensions, grid layouts, and same-row image relationships unless the user explicitly asks for a layout change.
- After writing a Feishu paper-card page, fetch it back and run the `paper-card-delivery` validators when practical: the Markdown validator for local drafts and the block-level validator for Feishu compact metadata and native image captions.
...
- Use `paper-card-delivery` as the fixed paper-card content standard. `PDF` must point directly to a PDF when available.
