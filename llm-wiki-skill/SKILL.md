...
For any single-paper deep dive / 深读 / 详细解析 work, also use [`paper-deep-dive`](../paper-deep-dive/SKILL.md). That skill is the single canonical delivery standard for PDF/HTML extraction, a main page containing only the complete `原文中译稿`, and manuscript fidelity. Do not add a `精读稿` unless the user explicitly requests it. Keep this skill's Research-Wiki placement rules, but do not define, relax, or finalize deep-dive deliverables from this skill alone.
...
飞书/Notion 组会整理的页面结构与章节顺序以 [`feishu-doc-workflow`](../feishu-doc-workflow/SKILL.md) 的 Meeting Notes 为准，保持稳定：`组会整理报告`、`Paper Cards`、`保守纪要`、`讨论脉络 / 老师反馈`、`会后头脑风暴`、`TODO`。报告在 `TODO` 结束，不要再写一节 `原始材料` 去重复子页。日期页只挂两个子页：`notes` 与 `原始材料：千问纪要与原文`（纪要和原文写进后一个页内，不要再并列三个子页）。千问纪要总览图作为开头内容补充，不新增章节。
