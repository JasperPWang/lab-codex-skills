...
8b. **Reference / citation verification gate (mandatory for both manuscripts):** build a single `[n] → PDF URL` map (prefer arXiv PDF), apply it to References (` | [url](url)`) and body (`[[n](url)]`), then verify URL correctness and cross-consistency as specified in Reference and Citation Link Contract. Do not mark either manuscript complete until this gate passes.
...
- **PDF link suffix (mandatory when a usable PDF URL can be found):** after the full reference text, append ` | ` (space, pipe, space) and then a hyperlink whose **display text equals the URL string itself**. Prefer an **arXiv PDF** URL of the form `https://arxiv.org/pdf/<id>` (with or without version, matching the cited work). If no arXiv PDF exists, use the best open PDF (OpenReview, CVF, PMLR, publisher OA, project page) in the same ` | [url](url)` form. If no PDF can be verified after search, keep the entry without a fake link and mark `PDF link: not found` in the verification log—do not invent URLs.
- **Canonical References line example (Markdown):**
  ```markdown
  [1] Author A, Author B. Paper title. Conference/Journal, year. | [https://arxiv.org/pdf/2401.12345](https://arxiv.org/pdf/2401.12345)
  ```
- On Feishu/Notion, render the same structure: `[n]` plain label + original English bibliography text + ` | ` + clickable URL whose visible text is the full URL.
...
- **Link the number, not the surrounding prose.** In Markdown, use the pattern that keeps outer brackets as citation chrome and makes the **number** the link text, targeting the **same PDF URL** recorded for that `[n]` in References:
  ...
  ```
- The body link target for `[n]` must be **identical** to the URL used after ` | ` for that same `[n]` in References (same string, not a different landing page). Prefer arXiv PDF over abstract HTML when both exist.
...
3. **Suffix format:** each linked entry uses ` | [url](url)` with display text == URL.
...
- Do not convert References into platform ordered-list numbering (`1.` `2.`). Keep original labels such as `[1]` visible in ordinary text, and append ` | [pdf-url](pdf-url)` when a verified PDF URL exists.
...
- References keep original `[n]` labels as plain text (not `1.` ordered lists), remain untranslated in Chinese manuscripts, and use ` | [pdf-url](pdf-url)` with display text equal to the URL when a verified PDF (prefer arXiv) exists.
