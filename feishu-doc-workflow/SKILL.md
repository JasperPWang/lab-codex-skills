...
- Use this default report structure (do not reorder or rename these sections): `组会整理报告`, `Paper Cards`, `保守纪要`, `讨论脉络 / 老师反馈`, `会后头脑风暴`, `TODO`, `原始材料`.
- The footer is **one** section named `原始材料`, with exactly two mentions: `notes` and `原始材料：千问纪要与原文`. Do not split the footer into `原始材料：个人笔记` plus `原始材料：千问纪要与原文`, and do not re-list `纪要` / `原文` as extra sibling links. On Notion, `replace_content` must preserve only those two children with `<page>` tags; extra sibling `<page>` blocks for `纪要` / `原文` recreate the bloated tree.
