...
description: Feishu/Lark document and wiki workflow for reading, editing, structuring, and verifying docs, wiki pages, paper cards, meeting notes, personal research cards, deep dives, images, and drive-native Markdown through the local `lark-cli`. Use when the user provides a feishu.cn/larksuite.com document or wiki URL, asks to read or edit 飞书/Feishu/Lark content, asks to maintain Feishu paper cards, meeting pages, 个人研究名片, meeting speaker attribution, mentions 飞书文档/飞书知识库, or needs authenticated Feishu document updates with hierarchy, image, and post-write verification rules.
---
...
4. For images, do not create or rewrite them by embedding raw XML `<img>` tags in document content. Use `docs +media-insert` with local image files and a text selection so Feishu creates native image blocks that keep the source image aspect ratio.
   - Figure captions without formulas must be stored as the native image caption, not as a separate ordinary paragraph below the figure. Captions containing inline formulas or formula-rendering fallbacks may remain as an immediately adjacent paragraph when native captions cannot preserve the formula, but this is a fidelity exception rather than the default.
...
- For paper-card figures, the complete Chinese figure caption belongs in the native image caption field. Do not leave `图 N｜完整中文图注：...` as a separate ordinary paragraph under the image unless native captions are unavailable and the fallback is explicitly reported. If the caption has no formula, this is a hard requirement; if the caption contains inline formulas or a formula fallback, an immediately adjacent caption paragraph is acceptable only to preserve formula fidelity.
- The caption content itself must satisfy `paper-card-delivery`: translate the official original caption completely, then add at most one short source tag such as `来源：用户截图`, `来源：HTML`, `来源：MinerU PDF 截图`, or `来源：PDF 截图` when useful. Do not replace the caption with a generated summary such as `方法或实验概览`, and do not add empty explanatory tails such as `用于说明论文的核心流程、输入输出关系和关键模块`, `原始 caption 已在图中保留`, or `便于回溯核验`.
...
- For new paper cards, when both a teaser / visual result / problem-context figure and a pipeline / method overview / architecture figure are available, insert both as one same-row native image group or grid after the metadata. Put the teaser first and the method/process figure second unless the paper's official visual order clearly suggests otherwise. Each image keeps its own native caption.
- Size a two-image paper-card group adaptively: preserve original pixel dimensions, aspect ratios, captions, and readable scale; set column/grid ratios according to the actual image shapes and text density; do not force equal-width columns if that makes the pipeline/method figure unreadable. If only the method/process figure is available, insert that single figure normally.
- Before uploading or inserting a paper-card figure, visually inspect the selected file or contact sheet and reject all-black / near-all-black / mostly dark low-information artifacts: web hero backgrounds, dark-mode browser screenshots, loading/failure frames, UI wrappers, or screenshots whose actual figure content is unreadable. Treat transparent-background assets as a likely black-render source: detect alpha/transparency when possible, composite the figure onto a white or paper-colored background, save a flattened PNG for Feishu upload, and inspect that flattened preview instead of trusting the raw transparent file. Allow dark backgrounds only when the official paper figure itself is intentionally dark and the content, labels, legends, and caption alignment remain readable. For a two-image teaser + method group, both images must pass this check independently; omit a weak black teaser rather than keeping it. If a figure appears black after Feishu upload or fetch-back, inspect the source pixels/dimensions/thumbnail and alpha channel; if it is a transparency compositing issue, replace the block with a flattened white-background PNG, otherwise replace it with a verified official asset, MinerU extraction, or PDF crop.
...
- In a Feishu paper card, place the native image block or same-row image group after the metadata paragraph link line (`PDF｜Project｜Code`) and `Dataset`, before the fixed bullet slots. Do not leave the image directly under the `####` title.
...
- Treat the user's `个人研究名片` project as the meeting speaker-identity layer. When a maintained research-card hub/page exists, fetch/search it before assigning meeting segments to people. Use cards to resolve aliases, research direction, current projects, recurring papers/terms, and likely responsibility areas; do not use them to invent facts that are absent from the notes or transcript.
- A personal research card should be enough to support meeting attribution: name and aliases, affiliation/role, research direction, active projects, representative papers or methods, recurring keywords, known collaboration context, Feishu card link, and verification status. Keep contact/private details out unless the user explicitly asks and the page already contains them.
- In `保守纪要` / `讨论脉络`, attach speaker attribution only when supported by personal notes, transcript labels, or high-confidence research-card matching. On first mention, link the person's name to their research-card page when available. If the match is weak, write `讲者待核验` or `可能为 <name>（待核验）`; never present a guessed speaker as fact.
- If the meeting reveals a new recurring collaborator or a research direction not yet covered by a card, add a TODO such as `补建 / 更新个人研究名片：<name or alias> — <reason>`. Do not create or update personal research cards during ordinary meeting cleanup unless the user asks for that scope.
...
- Feishu single-paper deep dives default to a fixed parent-plus-two-child structure. Do not scatter the durable reader-facing deep dive across multiple redundant child pages.
- Parent page title must be the official paper/article title only. Do not append status suffixes such as `中文`, `深度笔记`, `学习页`, `Deep Dive`, `阅读笔记`, or `解析`.
- The required child deliverables are `<paper short name>｜英文原文稿` and `<paper short name>｜原文中译稿`. The close-reading deliverable belongs on the parent page as `精读稿`, not as a default child page.
- When an official PDF or full-paper HTML is accessible, the two child deliverables are mandatory full manuscripts. Do not replace them with extraction-status pages, section summaries, selected excerpts, structure outlines, partial translations, or "English structured reader" pages. Long papers must be processed by section-level chunking and incremental Feishu writes until the child pages are complete.
- `<paper short name>｜英文原文稿` is the paper's original English text in source order. It is usually produced from the MinerU conversion draft, then checked against official HTML / LaTeX / PDF. It must preserve figure/table positions, formulas, captions, appendices, body citation markers, and a numbered References section. It is the working source for translation and can be deleted after translation if the user later asks.
- `<paper short name>｜原文中译稿` is the complete faithful Chinese manuscript. It preserves the source paper's section order, paragraph correspondence, formulas, figure/table positions, body citation markers, captions, numbered References section, and layout structure as much as Feishu allows. Translate the paper body, captions, and explanatory prose into Chinese, but keep the References / bibliography entries in their original English form. It also requires accurate terminology: important terms and proper nouns should be translated as `中文（English term）` on first use or where clarity is needed, while method/model/dataset names and mathematical symbols remain unchanged. Avoid leaving dense English technical terms untranslated in ordinary Chinese prose.
- In deep-dive manuscript pages, figure captions without formulas must be native image captions attached to the corresponding image. If a caption contains inline formulas that Feishu native captions cannot preserve, keep the caption as an immediately adjacent paragraph and preserve the exact formula source; do not treat this formula-preservation exception as permission to leave ordinary no-formula captions as loose paragraphs.
- Deep-dive figure captions must be faithful source captions: English original pages preserve the official original caption, and Chinese translation pages use the complete Chinese translation of that caption. Do not replace either with an agent-written summary, source-process note, or generic explanation of what the figure is used to show. If a source tag is needed on a working figure, keep it to the short controlled labels from `paper-card-delivery`.
...
- Parent-page `精读稿` is an analytical Chinese reading guide. It follows the source paper's original section order by default: Abstract, numbered main-paper sections, named subsections, conclusion, then appendices / supplementary material. Each part should be interpreted within the paper's own argumentative context: what the authors are doing, why it is needed, what mechanism or evidence appears there, and what remains unresolved. Do not reorganize the close-reading content into a purely thematic guide unless the user explicitly asks for a theme-based reader. Do not add a repeated per-section heading or paragraph such as "对你的 world model 研究意味着什么"; world-model / embodied-world-model implications belong in one final synthesis section after the source-order close reading.
- If any required child deliverable cannot be produced because the full paper source is inaccessible, parsing fails after trying MinerU and structured fallbacks, or licensing/copyright constraints apply, stop and tell the user what is blocking completion. Do not silently downgrade to a simplified reader or mislabeled excerpt page. Context length, paper length, translation workload, or one-turn time are not valid blockers; continue in chunks instead.
- Use a Feishu parent/child hierarchy:
  - Parent page: the deep-dive notes. This is the main reader-facing page and should contain the paper card / metadata, native Feishu `论文解析树` mind map, and `精读稿`: one-sentence conclusion, paper positioning, technical challenge, core insight, method / pipeline breakdown, losses or training details, datasets and evaluation, key experiments and ablations, limitations, related latest progress or critique, and implications for world-model / embodied-world-model research. In `精读稿`, keep the source-order close reading focused on the paper's own logic and original context; place user-specific world-model / embodied-world-model implications only in a final synthesis section instead of repeating them under every source section.
  - Child page 1: English original manuscript (`<paper short name>｜英文原文稿`), containing the original English paper text in source order, with formulas, figure/table positions, captions, appendices, citations, and references preserved as much as Feishu allows.
  - Child page 2: complete Chinese manuscript (`<paper short name>｜原文中译稿`), translated sentence by sentence where available and preserving formulas, figure/table placements, citations, captions, references, and the paper's original image order. Keep the References / bibliography entries as original English, not Chinese translation. Insert paper figures as native Feishu image blocks near their original figure captions whenever reliable official images are available. Follow the Formula / Equation Handling rules below; do not rebuild formulas from already-flattened Feishu plain text.
- For deep-dive figures, no-formula captions must be attached as native image captions. Only captions containing inline formulas, displayed formulas, or explicit formula fallbacks may remain as ordinary paragraphs, and they must be directly adjacent to the image they describe.
- For deep-dive figures, source labels are not explanatory prose. Use only short controlled tags such as `来源：HTML`, `来源：MinerU PDF 截图`, or `来源：PDF 截图` when a source tag is genuinely helpful; never add generic text such as `用于说明...`, `原始 caption 已在图中保留`, or `便于回溯核验`.
...
- References must be repaired before publishing. Do not leave bibliography entries as fragmented paragraphs or bullets; use a numbered bibliography, keep one reference per numbered item, preserve the original English bibliography text, and preserve body citation markers so the manuscript reads like the original paper. Do not translate paper titles, venues, publisher names, author names, page ranges, DOI/arXiv strings, or other reference-entry fields into Chinese.
...
- Remove obsolete process/status scaffolding from reader-facing parent pages, including `Source Extraction`, `Deep Dive Structure Status`, long local extraction inventories, and notes about MinerU availability unless the user explicitly asks for an audit log. Keep durable source links in a compact `来源` section.
- Verify final Feishu hierarchy and manuscript completeness: the parent notes page title is the official paper/article title, has exactly the intended child pages for `<paper short name>｜英文原文稿` and `<paper short name>｜原文中译稿`, and the parent page contains the paper card, native Feishu `论文解析树`, and `精读稿`. Fetch both child pages and check that they include the official source section list, main sections, appendices included in the PDF, figure/table captions, body citations, formulas, and numbered References. If either child page is partial, selected-excerpt-only, section-summary-only, or missing major source sections, report the package as WIP / incomplete rather than finished.
...
- In parent-page `精读稿` / close-reading content, use inline math `$...$` for symbols, variables, operators, and compact symbolic expressions such as `$S$`, `$G$`, `$l(S)$`, or `$l(S)-l(G)$`. Do not use inline code backticks for paper notation. Use plain prose for English technical terms unless they are part of a mathematical expression.
...
- Put the editable survey trees in the parent page by default. Include at least `General Goal Literature Tree`, `Challenge-Insight Tree`, and `Research Entry Points Tree` as native Feishu whiteboards. Do not add a duplicate bilingual editable outline or table below them by default, because the native mind map is already the editable artifact. Add a compact textual outline only when the board cannot be edited reliably, the user explicitly asks for a fallback, or the tree is too large to navigate in the board alone. Create a separate `Tree Maps` child page only if the user explicitly asks for separation or the trees are too large for the parent; even then, keep a compact tree-map summary in the parent.
...
- When repairing an older survey where the parent links to a separate `Tree Maps` child page, merge that content into the parent first: create or copy native Feishu whiteboards in the parent, remove the Tree Maps link from the parent `子页面` list, and leave the old child page untouched as a non-primary legacy copy unless the user explicitly asks to delete it. Do not copy an outline/table merely to duplicate a fully editable native mind map.
...
- Parent-page tree maps must contain at least three contentful editable maps by default: a `General Goal Literature Tree`, a `Challenge-Insight Tree`, and a `Research Entry Points Tree`. Their nodes should be based on the report's actual field structure, not generic labels. The research-entry-points map must be comprehensive enough to guide project selection: for each major direction, include problem framing, why-now / literature gap, core hypothesis, candidate route or pipeline, data / benchmark, evaluation signal, compute / cost when relevant, expected novelty type, risks / boundaries, and next experiment.
...
- Keep the native Feishu mind map itself editable. Do not add a second editable nested outline unless the board is static/unreliable, the user asks for a text fallback, or the map is too large to edit comfortably in the board alone.
...
- For `Research Entry Points Tree`, color by decision logic when custom colors are used: opportunity / direction in gold, problem framing in light blue, literature gap / why-now in wheat, core hypothesis in light green, route / pipeline in lavender, data / benchmark and evaluation in light gray, risk / boundary in light coral, and next experiment in light yellow.
...
- For Peng Sida / Learning Research style research maps, use three complementary trees by default:
  ...
  - `Research Entry Points Tree`: research opportunity -> problem formulation -> literature gap -> technical route -> data / evaluation -> risk / boundary -> next experiment.
- Verify after writing that only the intended board blocks remain, `flowchart LR` is absent when native mind map was requested, the fetched whiteboard content contains `@startmindmap`, and raw node text / exported preview show English newline Chinese for every mind-map node. For Feishu survey parent pages, also verify that all three expected maps are present unless the user explicitly excluded one.
