...
Each selected figure must pass visual QA independently. Reject all-black, near-all-black, or mostly dark low-information images when they are likely web hero backgrounds, dark-mode browser/page screenshots, failed/partial loading frames, UI wrappers, or screenshots whose actual figure content is unreadable. Treat transparent-background assets as a special case: transparency itself is not a reason to reject an otherwise correct official figure, but do not upload a transparent PNG/SVG/WEBP if it renders against black or becomes unreadable in the preview. Detect alpha/transparency when possible, composite the figure onto a white or paper-colored background, save a flattened PNG for Feishu upload, and verify the flattened preview before insertion. A dark or black background is acceptable only when it is clearly the official paper figure itself and the figure content, labels, legends, and caption alignment remain readable; when possible, crop away surrounding dark page/UI/background and prefer an official HTML asset, PDF figure crop, or MinerU extraction on a normal paper background. If a teaser is black/near-black or weak, omit it rather than weakening the card; the method/process figure remains sufficient.
...
Before batch-inserting figures into Feishu, create and visually inspect a contact sheet or equivalent preview of all selected images. For assets with alpha/transparency, include the white-background flattened preview in this sheet, not only the raw transparent file. Reject and replace any selected image that is an HTML banner, logo, author/avatar photo, OJS/publisher UI artifact, title-page render, mostly text-only PDF crop, result-only qualitative collage when a method figure exists, all-black / near-all-black / mostly dark with unreadable content, a transparent image rendered against black without flattening, a dark-mode web screenshot, a loading/failure frame, or a crop whose caption belongs to another figure. If an image appears black after Feishu upload or fetch-back, inspect its source pixels/dimensions/thumbnail and alpha channel; if it is a transparency compositing issue, replace it with a flattened white-background PNG, otherwise replace it with a verified official asset, MinerU extraction, or PDF crop.
...
- Selected figures are not all-black, near-all-black, failed-render frames, dark-mode web screenshots, or transparency-rendered black artifacts. Any transparent-background figure has been flattened onto a white or paper-colored background before Feishu upload unless the official source already provides a readable non-transparent version. In a two-figure group, both the teaser and method/process figure pass this check independently.
