...
- Verify final Feishu hierarchy: the parent notes page title is the official paper/article title, has exactly the intended child pages for `<paper short name>｜英文原文稿` and `<paper short name>｜原文中译稿`, and the parent page contains the paper card, native Feishu `论文解析树`, and `精读稿`.
