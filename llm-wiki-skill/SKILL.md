...
飞书/Notion 组会整理的页面结构与章节顺序以 [`feishu-doc-workflow`](../feishu-doc-workflow/SKILL.md) 的 Meeting Notes 为准，保持稳定：`组会整理报告`、`Paper Cards`、`保守纪要`、`讨论脉络 / 老师反馈`、`会后头脑风暴`、`TODO`、`原始材料`。日期页只挂两个子页：`notes` 与 `原始材料：千问纪要与原文`（纪要和原文写进后一个页内，不要再并列三个子页）。千问纪要总览图作为开头内容补充，不新增章节。
