...
```text
对这篇论文做完整 deep dive，必须满足 paper-deep-dive 交付标准：页面属性用中文标题；正文开头是普通段落形式的英文标题、英文作者与单位，以及最新 arXiv PDF、Project Page、Code 三条链接，英文标题下面不放中文标题；随后只放完整原文中译稿，不默认写精读稿；公式保留 LaTeX；参考文献与正文引用按 deep-dive 链接合同；目标平台用对应 *-doc-workflow 写入并回读验证。
<PDF_OR_ARXIV_OR_PROJECT_URL>
目标：<FEISHU_OR_NOTION_URL_OR_OBSIDIAN_PATH>
```
